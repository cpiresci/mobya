window.RentalHost = (() => {
  ...
  function bookingCard(b){
    ...
    return`<div id="hbcard-${esc(b.id)}" style="background:var(--s2);border:1px solid ${sm.border||'var(--border)'};border-radius:12px;padding:20px;display:flex;flex-direction:column;gap:12px;transition:border-color .2s" onmouseover="this.style.borderColor='rgba(0,245,255,.25)'" onmouseout="this.style.borderColor='${sm.border||'var(--border)'}'">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        <div style="display:flex;align-items:center;gap:9px"><span style="font-size:1.4rem">🗝️</span><div><div style="font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:2px;color:var(--neon)">RESERVA #${esc(b.id.slice(-6).toUpperCase())}</div><div style="font-size:.7rem;color:var(--muted);font-family:'JetBrains Mono',monospace">${fmtD(b.createdAt)}</div></div></div>
        ${badge(b.status)}
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        ...
        <div style="font-size:.72rem;color:var(--muted)"><div style="color:var(--text-dim);margin-bottom:2px">TOTAL</div><div style="font-family:'JetBrains Mono',monospace;color:var(--gold)">${fmtBRL(b.totalAmount)}</div></div>
        <div style="font-size:.72rem;color:var(--muted)"><div style="color:var(--text-dim);margin-bottom:2px">SEU REPASSE</div><div style="font-family:'JetBrains Mono',monospace;color:var(--green)">${fmtBRL(b.hostPayout)}</div></div>
        <div style="font-size:.72rem;color:var(--muted)"><div style="color:var(--text-dim);margin-bottom:2px">COMISSÃO MOBYA</div><div style="font-family:'JetBrains Mono',monospace">${fmtBRL(b.commissionAmount)}</div></div>
      </div>
      ${b.renterNotes?`<div style="font-size:.78rem;color:var(--text);background:rgba(0,245,255,.06);border:1px solid rgba(0,245,255,.15);border-radius:7px;padding:9px 12px;line-height:1.5"><span style="color:var(--neon);font-family:'JetBrains Mono',monospace;font-size:.65rem">📝 OBS.</span><br>${esc(b.renterNotes)}</div>`:''}
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:2px">
        ...
        ${canI?`<button onclick="RentalHost.checkin('${esc(b.id)}')" style="flex:1;background:linear-gradient(135deg,var(--neon),#0077b6);color:#000;border:none;border-radius:8px;padding:10px;font-weight:700;font-size:.8rem;cursor:pointer;font-family:'Space Grotesk',sans-serif">🚗 Check-in</button>`:''}
        ${canO?`<button onclick="RentalHost.checkout('${esc(b.id)}')" style="flex:1;background:linear-gradient(135deg,var(--gold),#d97706);color:#000;border:none;border-radius:8px;padding:10px;font-weight:700;font-size:.8rem;cursor:pointer;font-family:'Space Grotesk',sans-serif">🏁 Devolução</button>`:''}
  ...
  async function render(){
    ...
    try{
      const [rP,rC,rA,rCo,rCa]=await Promise.all([API.rental.hostBookings({status:'PENDING',limit:50}),API.rental.hostBookings({status:'CONFIRMED',limit:50}),API.rental.hostBookings({status:'ACTIVE',limit:50}),API.rental.hostBookings({status:'COMPLETED',limit:50}),API.rental.hostBookings({status:'CANCELLED',limit:50})]);
      const get=r=>r?.data?.bookings||r?.data||[];
      _bookings={PENDING:get(rP),CONFIRMED:get(rC),ACTIVE:get(rA),COMPLETED:get(rCo),CANCELLED:get(rCa)};
      const totalRecv=_bookings.COMPLETED.reduce((s,b)=>s+(b.hostPayout||0),0);
      const pendRecv=[..._bookings.CONFIRMED,..._bookings.ACTIVE].reduce((s,b)=>s+(b.hostPayout||0),0);
      const kpiEl=document.getElementById('rh-kpis');
      if(kpiEl)kpiEl.innerHTML=[{label:'AGUARDANDO',value:String(_bookings.PENDING.length),color:'var(--gold)',icon:'⏳'},{label:'EM CURSO',value:String(_bookings.ACTIVE.length),color:'var(--neon)',icon:'🚗'},{label:'A RECEBER',value:fmtBRL(pendRecv),color:'var(--green)',icon:'💸'},{label:'RECEBIDO',value:fmtBRL(totalRecv),color:'var(--q3)',icon:'💰'}].map(k=>`<div style="background:var(--s2);border:1px solid var(--border);border-radius:10px;padding:18px"><div style="font-size:1.3rem;margin-bottom:8px">${k.icon}</div><div style="font-family:'JetBrains Mono',monospace;font-size:.6rem;color:var(--muted);letter-spacing:1px;margin-bottom:6px">${k.label}</div><div style="font-family:'Bebas Neue',sans-serif;font-size:1.5rem;color:${k.color}">${k.value}</div></div>`).join('');
      renderList('PENDING');
