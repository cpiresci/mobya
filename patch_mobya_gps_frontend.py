#!/usr/bin/env python3
"""
MOBYA — Patch Frontend: GPS Tracking (Leaflet + Socket.io client)
Roda em: ~/mobya-master
"""
import os, sys

BASE = os.path.expanduser('~/mobya-master')

# ── gps-tracking.js (lê do arquivo gerado) ───────────────────
GPS_JS = r"""window.GPSTracking = (() => {
  const API_BASE = window.API_URL || 'https://mobya-app.onrender.com/api/v1';
  ...
  function initMap(id){if(map){map.remove();map=null;}map=L.map(id,{zoomControl:true,attributionControl:false});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(map);map.setView([-15.788,-47.879],5);return map;}
  function updateMarker(role,lat,lng,label){const isP=role==='PROVIDER';const e=isP?'🔧':'📍';const c=isP?'#3b82f6':'#10b981';if(markers[role]){markers[role].setLatLng([lat,lng]);}else{markers[role]=L.marker([lat,lng],{icon:makeIcon(e,c)}).addTo(map).bindPopup(`<strong>${label}</strong>`);}if(markers.USER&&markers.PROVIDER){const pts=[markers.USER.getLatLng(),markers.PROVIDER.getLatLng()];if(routeLine){routeLine.setLatLngs(pts);}else{routeLine=L.polyline(pts,{color:'#3b82f6',weight:3,dashArray:'8 6',opacity:.7}).addTo(map);}map.fitBounds(L.latLngBounds(pts),{padding:[40,40]});}else{map.setView([lat,lng],15);}}
  function connectSocket(token){if(socket)socket.disconnect();socket=io(WS_BASE+'/gps',{auth:{token},transports:['websocket','polling'],reconnectionAttempts:10,reconnectionDelay:2000});socket.on('connect',()=>updateConnectionStatus('online'));socket.on('disconnect',()=>updateConnectionStatus('offline'));socket.on('connect_error',(e)=>{console.warn('[GPS]',e.message);updateConnectionStatus('reconectando');});socket.on('session_joined',({role,session})=>{myRole=role;updateSessionStatus(session.status);updateProviderControls(role);addChatMessage({text:`Você entrou como ${role==='USER'?'Cliente':'Prestador'}.`,from:'Sistema',role:'SYSTEM',ts:Date.now()});});socket.on('location_update',({role,lat,lng,name,eta})=>{updateMarker(role,lat,lng,name||role);if(eta&&role==='PROVIDER')updateETA(eta);});socket.on('status_changed',({status})=>{updateSessionStatus(status);const s=STATUS_LABEL[status];if(s)Toast.show(`${s.icon} ${s.text}`,status==='CONCLUIDO'?'ok':'info');if(['CONCLUIDO','CANCELADO'].includes(status))stopWatchingLocation();});socket.on('participant_online',({role,name})=>addChatMessage({text:`${name} entrou.`,from:'Sistema',role:'SYSTEM',ts:Date.now()}));socket.on('participant_offline',({role})=>addChatMessage({text:`${role==='USER'?'Cliente':'Prestador'} saiu.`,from:'Sistema',role:'SYSTEM',ts:Date.now()}));socket.on('chat_message',(msg)=>addChatMessage(msg));socket.on('error',({code})=>Toast.show(`Erro GPS: ${code}`,'error'));}
  function joinSession(sid){sessionId=sid;if(!socket?.connected)return;socket.emit('join_session',{sessionId:sid});}
  function startWatchingLocation(){if(!navigator.geolocation){Toast.show('GPS não disponível.','warn');return;}const opts={enableHighAccuracy:true,maximumAge:3000,timeout:10000};watchId=navigator.geolocation.watchPosition((pos)=>{const{latitude:lat,longitude:lng,heading,speed,accuracy}=pos.coords;if(socket?.connected)socket.emit('send_location',{lat,lng,heading,speed,accuracy});if(myRole)updateMarker(myRole,lat,lng,'Você');},(err)=>console.warn('[GPS]',err.message),opts);updateGPSStatus('ativo');}
  function stopWatchingLocation(){if(watchId!==null){navigator.geolocation.clearWatch(watchId);watchId=null;}updateGPSStatus('parado');}
  ...
  function updateConnectionStatus(s){const el=document.getElementById('gpsConnectionStatus');if(!el)return;const m={online:['● Online','#10b981'],offline:['● Offline','#ef4444'],reconectando:['◌ Reconectando','#f59e0b']};const[t,c]=m[s]||['',''];el.textContent=t;el.style.color=c;}
  ...
  function updateETA({distanceKm,etaMinutes}){const el=document.getElementById('gpsETA');if(!el)return;el.innerHTML=`<span style="color:#3b82f6">📍 ${distanceKm}km • ~${etaMinutes} min</span>`;}
  ...
  return{render,openTracking,createSession,joinSession,setStatus,sendChatMessage,startWatchingLocation,stopWatchingLocation};
})();
"""

# ── PATCHES index.html ────────────────────────────────────────
# 1. Leaflet CSS no <head>
OLD_HEAD = '</head>'
NEW_HEAD = '''  <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
</head>'''

# 2. Scripts antes do </body> — Leaflet + Socket.io client + gps-tracking
OLD_SCRIPTS = '<script src="js/admin-approval.js?v=20260618"></script>'
...
<script src="js/gps-tracking.js?v=20260618"></script>'''

# 3. Sidebar item GPS
OLD_SIDEBAR_ADMIN = '<div class="sb-item sb-role-admin" data-page="admin-aprovacao"'
NEW_SIDEBAR_GPS   = '''<div class="sb-item" data-page="gps-tracking">📡 GPS Tracking</div>
      <div class="sb-item sb-role-admin" data-page="admin-aprovacao"'''

# ── PATCH app.js ──────────────────────────────────────────────
OLD_APP = "  'admin-aprovacao':() => {"
NEW_APP = """  'gps-tracking':() => {
...
def patch(path, old, new, label):
    if not os.path.exists(path):
        errors.append(f'NÃO ENCONTRADO: {path}'); return
    with open(path) as f: s = f.read()
    if old not in s:
        print(f'SKIP ({label}): trecho não encontrado'); return
    with open(path, 'w') as f: f.write(s.replace(old, new, 1))
    print(f'OK: {label}')

# gps-tracking.js
gps_path = os.path.join(BASE, 'js/gps-tracking.js')
if os.path.exists(gps_path):
    print('SKIP: js/gps-tracking.js já existe')
else:
    with open(gps_path, 'w') as f: f.write(GPS_JS)
    print('OK: js/gps-tracking.js')

html = os.path.join(BASE, 'index.html')
patch(html, OLD_HEAD,          NEW_HEAD,          'index.html — Leaflet CSS')
patch(html, OLD_SCRIPTS,       NEW_SCRIPTS,       'index.html — Leaflet JS + Socket.io + gps-tracking.js')
patch(html, OLD_SIDEBAR_ADMIN, NEW_SIDEBAR_GPS,   'index.html — sidebar GPS Tracking')

patch(os.path.join(BASE, 'js/app.js'), OLD_APP, NEW_APP, 'app.js — rota gps-tracking')
...
print('\nPatches OK. Commitando...')
os.chdir(BASE)
os.system('git add js/gps-tracking.js js/app.js index.html')
os.system('git commit -m "feat: GPS tracking tempo real (Leaflet + Socket.io)"')
os.system('git push')
print('DONE: frontend GPS deployado!')
